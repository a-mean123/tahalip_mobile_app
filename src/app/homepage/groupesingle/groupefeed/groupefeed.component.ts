...
@Component({
  selector: 'app-groupefeed',
  templateUrl: './groupefeed.component.html',
  styleUrls: ['./groupefeed.component.scss'],
})
export class GroupefeedComponent implements OnInit {
...
if(this.comment.text.length === 0 && this.fileInput.nativeElement.files.length === 0){
  ...
  getPosts(){
    this._data.getPostByGroupeId(this.id, 0).subscribe(
      res=>{
        this.posts = res;
